...
class AQIDataset(Dataset):
    def __init__(self, file_paths):
        self.data = self.load_one_day(file_paths)

    def load_one_day(self, file_paths):
        pollutant_arrays = []
        ...
        stacked = np.stack(pollutant_arrays, axis=0)  # (6, 24)
        flattened = stacked.reshape(-1)               # (144,)
        return np.array([flattened])                 # dataset of size 1
...
def extract_min_max(file_paths):
    ...
    for fp, pollutant in zip(file_paths, pollutants):
        df = pd.read_csv(fp)
        aqi_col = [c for c in df.columns if "AQI" in c][0]
        vals = df[aqi_col].values.astype(np.float32)
        aqi_min[pollutant] = vals.min()
        aqi_max[pollutant] = vals.max()
...
def generate_year_of_aqi(model, latent_dim=8):
    ...
    samples = samples.numpy().reshape(365, 6, 24)
    return samples
...
def denormalize_and_clamp(samples, aqi_min, aqi_max):
    ...
    for i, pollutant in enumerate(pollutants):
        vmin = aqi_min[pollutant]
        vmax = aqi_max[pollutant]
        denorm[:, i, :] = samples[:, i, :] * (vmax - vmin) + vmin
...
def flatten_to_hourly_csv_with_total(samples):
    ...
    for day_idx in range(365):
        for hour in range(24):
            co, no2, o3, pm25, pm10, so2 = samples[day_idx, :, hour]
